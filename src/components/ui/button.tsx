...
function Button({
  ...
  return (
    <Comp
      ...
      {...props}
    >
      {/* Spinner kalau loading */}
      {isLoading && (
        <Loader2
          className={cn(
            'animate-spin',
            hasText ? 'mr-1.5' : '', // kasih spacing hanya kalau ada teks
          )}
        />
      )}

      {!isLoading && leftIcon && (
        <span className={cn('flex items-center', hasText && 'mr-1.5')}>
          {leftIcon}
        </span>
      )}
      ...
      {!isLoading && rightIcon && (
        <span className={cn('flex items-center', hasText && 'ml-1.5')}>
          {rightIcon}
        </span>
      )}
    </Comp>
